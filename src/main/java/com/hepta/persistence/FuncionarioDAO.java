...
public class FuncionarioDAO {
	...
	public List<Funcionario> getAll() throws Exception{
	 ...
	 try {
		db.getTransaction().begin();
		Query selectAll = db.createQuery("FROM Funcionario f "
				+ "JOIN FETCH f.setor"); //SETOR É UMA PROPRIEDADE DE FUNCIONARIO POR ISSO REFERENCIA F.SETOR
	...
	public Funcionario find(int id) throws Exception{
		...
		try {
		db.getTransaction().begin();
		Query find = db.createQuery("FROM Funcionario f "
				+ "JOIN FETCH f.setor WHERE Id_FUNCIONARIO = " + id);
