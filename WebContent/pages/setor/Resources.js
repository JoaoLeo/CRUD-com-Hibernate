function criarSetor(){
	...
	var urlCriar = "http://localhost:8080/hibernatecrud/rest/setor/criar";
...
function listarSetores(){
    ...
    request.onload = () => {
        ...
        resposta.forEach(r => {
           bodyHTML += `  <tr>  <td> ${r.id} </td> <td> ${r.nome} </td>  
           <td>  <div style = "display: flex;"> <form> <input type ="hidden" value = ${r.id}> <a href = #">  <button type = "button"  onclick = 'pegarId(${r.id})'>  Alterar     </button> </a> </form>
              <form onsubmit = "excluirSetor(${r.id})"> <input type ="hidden" value = ${r.id}> <button> Excluir </button>  </form> </td>   </td> </div> </tr>`          	
        });
...
function pegarId(id) {    	
    window.location.href = 'http://localhost:8080/hibernatecrud/pages/setor/atualizarSetor.html?id=' + id
...
function carregarInformacoes(){
    ...
    var urlListagem = "http://localhost:8080/hibernatecrud/rest/setor/buscar/" + id;
...
function atualizarSetor(){
    ...
    var urlAtualizar = "http://localhost:8080/hibernatecrud/rest/setor/atualizar/" + id;
...
function buscarUmSetor(id){
	...
	bodyDiv.innerHTML = "";
	var urlBusca = `http://localhost:8080/hibernatecrud/rest/setor/buscar/${id}`
	...
	requestBusca.onload = () => {
		...
		resposta.forEach(r => {
			bodyDiv.innerHTML += `<h3>  Informações do setor </h3> <li> ID: ${r.id} </li>`
...
function excluirSetor(id){
	...
	if(confirmDelete === true){
		var urlDelete = "http://localhost:8080/hibernatecrud/rest/setor/excluir/" + id;
